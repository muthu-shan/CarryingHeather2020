...
from smile_detector import detect
    ...
    def find_motion(callback, camera_port=0, show_video=False):
        ...
        while True:
            ...
            if c is not None:
                ...
                # smile detector is incorporated here
                smile_flag = detect(gray,frame)
                callback(c, frame,smile_flag)
...
class Turret(object):
    ...
    def __move_axis(self, contour, frame):
        ...
        # fire if necessary
        # fire only when not in friendly mode and not smiling
        if not self.friendly_mode and not smile_flag:
